...
| 命令 | 作用 |
| ------ | ------ |
| `ls [引用] [-l]` | 列笔记本/文档; 引用=笔记本名\|id\|路径 (必须真实存在); 通配 `ls /*/AI伴学` 显式模糊 |
| `tree <doc>` | 标题树/大纲 |
...
> **引用协议 (全部命令统一, Linux 直觉)**:
> - **路径必须真实存在**, 不存在即报错 (`ls /AI伴学` → 找不到); **模糊用显式通配**: `*` 任意层级 (含零层), `?` 单字符, 如 `ls /*/AI伴学`
> - 路径形式: **完整路径** `/工作/日志/2026/AI伴学` (带笔记本名, 带不带前导 `/` 均可) 或 **hpath** `/日志/2026/AI伴学`
> - **标题** (无 `/` 引用): 精确同名优先, 多匹配列出候选并报错 (防误操作)
> - 通配多命中为正常结果 (列表命令列出全部; 单目标命令列候选报歧义)
> - find/grep/which -v 输出统一为完整路径, 可直接喂回 which/ls。
> 组合示例: `siyuan cat /工作/日志/2026/AI伴学/关键流程/整体架构风格与微服务拆分`、`siyuan ls 工作 | siyuan grep 调课`。旧命令名 list/read/get/outline/search/notebooks 保留为别名。
