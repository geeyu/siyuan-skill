...
| 命令 | 作用 |
| ------ | ------ |
| `ls [引用] [-l]` | 列笔记本/文档; 引用=笔记本名\|id\|任意层级路径 (`ls /工作/日志/2026/AI伴学`、`ls /AI伴学` 部分路径均可, 多匹配列候选) |
| `tree <doc>` | 标题树/大纲 |
...
> **引用协议 (全部命令统一)**: `<doc>` 可以是 id / 标题 / 路径, 按解析链自动匹配:
> 1. **完整路径** `/工作/日志/2026/AI伴学` (带笔记本名, 带不带前导 `/` 均可)
> 2. **hpath** `/日志/2026/AI伴学` (无笔记本名, 精确匹配)
> 3. **部分路径** `/AI伴学`、`/关键流程` (后缀匹配, 唯一即命中)
> 4. **标题** (精确同名优先)
> 多匹配一律列出候选并报错 (防误操作); find/grep/which -v 输出统一为完整路径, 可直接喂回 which/ls。
> 组合示例: `siyuan cat /AI伴学/关键流程/整体架构风格与微服务拆分`、`siyuan ls 工作 | siyuan grep 调课`。旧命令名 list/read/get/outline/search/notebooks 保留为别名。
