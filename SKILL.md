...
9. **批量整理文档 (⚠ 高频事故区)**: ① `rm <目录>` 级联删除全部子文档 — 先移出再删; ② 同名文档很常见, 批量循环**用 id 不用标题**; ③ rename/建目录前先 `find` 预检重名; ④ **空内容操作**: `edit --replace` 空内容会报错且不修改文档, `touch --data ""` 建空文档; ⑤ 文档异常就"新起文件删旧", 别修数据库。详见 conventions.md §14-17。
